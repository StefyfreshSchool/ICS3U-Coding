...
public class DoubleArraySequence {
   ...
   public DoubleArraySequence(DoubleArraySequence src) {
      this.manyItems = src.getCapacity();
      this.currentIndex = 0;
   ...
   public void addAfter(double d) {
      ...
      if (manyItems == currentIndex) data[currentIndex] = d;
      else data[currentIndex + 1] = d;
      if (manyItems != currentIndex) currentIndex++;
      manyItems++;
   }
   ...
   public void addBefore(double element) {
      ensureCapacity(manyItems + 1);
      if (manyItems == currentIndex) currentIndex = 0;
   ...
   public void addAll(DoubleArraySequence addend) {
      ...
      addend.start();
      for (int i = manyItems; i < addend.size() + manyItems; i++){
    ...
    **/
   public void advance() {
      if (!isCurrent()) throw new IllegalStateException("No current element - advance() cannot be called.");
      currentIndex++;
   }
   ...
   public static DoubleArraySequence catenation(DoubleArraySequence s1, DoubleArraySequence s2) {
      if (s1 == null || s2 == null) throw new NullPointerException("Both objects must me non-null!");
      
      int total = s1.size() + s2.size();
      DoubleArraySequence out = new DoubleArraySequence(total);
      for (int i = 0; i < s1.size(); i++){
         out.addAfter(s1.data[i]);
      }
      for (int i = s1.size(); i < total; i++){
         out.addAfter(s2.data[i - s1.size()]);
      }
   ...
   public void ensureCapacity(int minimumCapacity) {
      ...
      int mul = minimumCapacity;
      if (minimumCapacity == data.length + 1) mul = data.length * 2;
      double[] temp = new double[mul];
   ...
   public double getCurrent() {
      if (!isCurrent()) throw new IllegalStateException("No current element - getCurrent() cannot be called.");
